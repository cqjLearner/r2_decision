...
/********************************************************
函数功能：将机器人的底盘速度，打包发送给下位机
入口参数：vx,vy,vw
出口参数：无
********************************************************/
void sendVelDataFloat(float vx,float vy,float vw,unsigned char crtlFlag)
{
    //发送数据缓存区
    ...
    for(i = 0;i < 4; i++)
    {
        buf[i + 3] = Vx.data[i];    //buf[3]  buf[4] buf[5] buf[6]
        buf[i + 7] = Vy.data[i];   //buf[7]  buf[8]  buf[9]  buf[10]
        buf[i + 11] = Vw.data[i];   //buf[11]  buf[12]  buf[13]  buf[14]
    }
    //预留控制位
//    buf[3 + length - 1] =crtlFlag;    //buf[15]不预留控制位 
   buf[3 + length - 1] = crtlFlag;    //buf[15]预留控制位 

    //设置校验位
    // buf[3+length] = serial_get_crc8_value(buf,3 + length); //buf[15]
    buf[3 + length] = serial_get_crc8_value(buf,3 + length); //buf[16]
    //设置消息尾
    for(i=0;i<2;i++)
    {
        // buf[3+length+1+i] = header[i];      //buf[16]   buf[17]
        buf[3+length+1+i] = header[i];      //buf[17]   buf[18]
    }
    //通过串口发送数据
    boost::asio::write(sp,boost::asio::buffer(buf));
}
...
bool receiveData(unsigned char &ctrlFlag)
{
            char i, length =0;   //标志位
            unsigned char checkSum;  //校验码标志位
            unsigned char buf[150]={0};
           ...
           //数据长度
           length = buf[2];     //buf[2]   length=1(only ctrlflag)
           ///检查消息检验位
           checkSum = serial_get_crc8_value(buf,3+length);   //buf[4]
           if(checkSum != buf[3+length])
           {
            ROS_ERROR("Receive data check sum error!");
            return false;
